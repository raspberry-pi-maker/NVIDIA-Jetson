...
def visualize_joints(image, pose):
    ...
    maxx = width - 2 * marker_size
    maxy = height - 2 * marker_size
    ...
    for p_idx in range(num_joints):
        ...
        if check_point(cur_x, cur_y, minx, miny, maxx, maxy):
            visim = _npcircle(visim, cur_x, cur_y, marker_size, colors[p_idx], 0.0)
    return visim
...
def show_heatmaps(cfg, img, scmap, pose, cmap="jet"):
    ...
    for pidx, part in enumerate(all_joints):
        ...
        scmap_part = Image.fromarray(np.uint8(scmap_part))
        width, height = scmap_part.size
        scmap_part = scmap_part.resize((int(width *8.0), int(height *8.0)))
